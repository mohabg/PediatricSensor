...
- (void)parseData:(NSData*)data forDevice:(TDTempoDisc*)device {
    ...
    for (NSInteger i=0; i<length; i+=2) {
        if (d[i] == kDataTerminationValue) {
            ...
            [self didFinishDownloadForType:_currentDownloadType device:device];
            break;
        }
        else {
...
- (void)didFinishDownloadForType:(DataDownloadType)type device:(TDTempoDisc*)device {
    [self saveData:_currentDataSamples type:type device: device];
    ...
    switch (type) {
        ...
        downloadType = DataDownloadTypeFinish;
        stringToWrite = kDataStringTransmitEnd;
        [device setLogCount:_logCounter];
        device.lastDownload = [NSDate date];
...
- (void)saveData:(NSArray*)data type:(DataDownloadType)type device:(TDTempoDisc*)device {
...
//        [device deleteOldData:readingType context:[(AppDelegate*)[UIApplication sharedApplication].delegate managedObjectContext]];
//        [NSThread sleepForTimeInterval: 1.0];
//
//        NSLog(@"Writing new data");
//        [device addData:data forReadingType:readingType startTimestamp:_startingTimeStamp interval:[(TempoDiscDevice*)[TDSharedDevice sharedDevice].selectedDevice timerInterval].integerValue context:[(AppDelegate*)[UIApplication sharedApplication].delegate managedObjectContext]];
//        [NSThread sleepForTimeInterval: 1.0];
//
    }
...
- (void)downloadDataForDevice:(TDTempoDisc *)device withCompletion:(void (^)(BOOL))completion {
    _currentDataSamples = [NSMutableArray array];
    ...
    [[LGCentralManager sharedInstance] scanForPeripheralsByInterval:kDeviceReConnectTimeout completion:^(NSArray *peripherals) {
        for (LGPeripheral *peripheral in peripherals) {
            if ([peripheral.UUIDString isEqualToString:device.peripheral.UUIDString]) {
                ...
                [device.peripheral connectWithCompletion:^(NSError *error) {
                    [timer invalidate];
                    timer = nil;
                    ...
                    if (!error) {
                        ...
                        [device.peripheral discoverServicesWithCompletion:^(NSArray *services, NSError *error2) {
                            if (!error2) {
